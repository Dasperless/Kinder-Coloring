...
vector<NodoPais *> DivideAndConquer::dividirGrafo(vector<NodoPais *> vectorNodos)
{
	if (vectorNodos.size() == 1) //Caso de parada
	{
		...
		{ // Recorre cada color en la lista para verificar cual color colocar
		...
		if (pais->colorPais == "fill:#f2f2f2;fill-rule:evenodd")
		{ // Si no hay color disponible se deja el original y se retorna un vector vacio
